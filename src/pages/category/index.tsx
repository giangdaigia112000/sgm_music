...
const Category = () => {
    const [listData, setListData] = useState<Category[]>([] as Category[]);
    ...
    useEffect(() => {
        (async () => {
            setLoadingInit(true);
            try {
                const resData = await axiosClient.post("/api/admin/category", {
                    _method: "get",
                });
                setListData(resData.data as Category[]);
            } catch (error) {
                notiError("Lỗi!!!");
            } finally {
    ...
    const handleRepair = (id: number) => {
        ...
        setIsAddOrFix(false);
        setIsOpenModal(true);
        const category = listData.filter((category) => category.id === id)[0];
